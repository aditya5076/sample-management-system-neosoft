...
class SampleEfficiency implements FromCollection, WithHeadings
{
    ...
    */
    ...
    {
        $this->query_array = RequestsTable::select('request_no','unique_sku_id','quality_name','design_name','shade_name','created_at','due_date','delivery_date',DB::raw("DATEDIFF(created_at,delivery_date) as delay"));
        if(!empty($this->from_date) && !empty($this->to_date)){
            $this->query_array = $this->query_array->whereBetween('created_at', [$this->from_date. " 00:00:00", $this->to_date. " 23:59:59"]);
        }
        $this->query_array = $this->query_array->orderBy('created_at', 'DESC')->get(); 
