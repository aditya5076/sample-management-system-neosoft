...
class RequestListingController extends Controller
{
    ...
    (SELECT location_master.location_name, location_master.id
        ...
        // $this->requests_array = $this->requests_array->get();
    ...
    protected function getOutwardModalData(Request $request)
    {
        ...
        foreach ($request->checked_entries as $key => $value) {
            $data[] = DB::table('requests')
                ...
                LEFT JOIN outward ON inward.request_id = outward.request_id
                    AND inward.unique_sku_id = outward.unique_sku_id
                    AND inward.location_id = outward.location_id
                GROUP BY
                    inward.unique_sku_id, inward.location_id, inward.request_id) as inward_outward'),
                    function ($join) {
                ...
                ->select(
                    ...
                    'location_id',
                    'requests.requirement',
                    DB::raw('COALESCE(inward_outward.total_quantity, 0) as total_quantity'),
                    DB::raw('COALESCE(inward_outward.total_outward_quantity, 0) as total_outward_quantity'),
                    DB::raw('(COALESCE(inward_outward.total_quantity, 0) - COALESCE(inward_outward.total_outward_quantity, 0)) as available_quantity')
                )
                ->having('available_quantity', '>', self::outward_ineligible)
                ->having('total_quantity', '>', self::outward_ineligible)
                ->where('location_id', $value['location_id'])
                ->where('requests.unique_sku_id', $value['sku_id'])
                ->where('requests.id', $value['request_id'])
                ->get()
                ->toArray();
        }
